...
export const Navbar: React.FC = () => {
  ...
  const scrollToSection = (id: string) => {
    ...
    // but usually requires a hash-link handler. 
    // For simplicity in this stack, we just link to Home.
  };
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 md:px-6 py-4 md:py-6 pointer-events-none">
      <div className="max-w-7xl mx-auto glass-card rounded-full px-4 md:px-8 py-3 flex items-center justify-between pointer-events-auto">
        ...
        <div className="hidden md:flex items-center gap-8 lg:gap-10">
          <Link to="/" className="text-sm font-bold text-slate-600 hover:text-primary transition-colors">首页</Link>
          ...
          {isHome ? (
            ...
            <>
              <Link to="/#features" className="text-sm font-bold text-slate-600 hover:text-primary transition-colors">功能特性</Link>
              <Link to="/#process" className="text-sm font-bold text-slate-600 hover:text-primary transition-colors">使用流程</Link>
            </>
          )}
